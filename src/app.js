...
//----------------------- PARTE PRACTICA -----------------------------
  ...
  // a) para recorrer ca elemento, utilizo la función flecha.

  let inventariosintrampa = inventario.filter(item => item !== 'trampa');
  console.log('inventario eliminando trampa:', inventariosintrampa);

  // b)

  inventariosintrampa.push('super estrella')
  console.log('inventariosintrampa agregando estrella:', inventariosintrampa);
  
  // c)

  let inventariomayuscula = inventariosintrampa.map(item => item.toUpperCase());
  console.log('inventario en mayusculas:', inventariomayuscula);

  // d) 

  let [primerItem, segundoItem] = inventariomayuscula;
...
for (let item of inventario) {
  if (item === 'trampa') {
    console.log('¡Cuidado hay una tampa');
